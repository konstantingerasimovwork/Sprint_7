...
class TestAcceptOrder:
    ...
    def test_accept_order(self, login_id, order_id):
        self.accept_order.put_request(login_id, order_id)
        ...
        self.accept_order.finish_order(order_id)
        self.accept_order.delete_courier(login_id)
    ...
    def test_accept_order_without_order_id(self, login_id):
        self.accept_order.put_request_without_order_id(login_id)
        response_status_code = self.accept_order.check_response_status_code()
        response_text = self.accept_order.check_response_text()
    ...
    def test_accept_order_without_login_id(self, order_id):
        self.accept_order.put_request_without_login_id(order_id)
        response_status_code = self.accept_order.check_response_status_code()
        response_text = self.accept_order.check_response_text()
        ErrorSchema.parse_obj(response_text)
        assert response_status_code == 400 and response_text == {
    ...
    def test_accept_order_with_non_existent_order_id(self, login_id):
        self.accept_order.put_request(login_id, 0)
        response_status_code = self.accept_order.check_response_status_code()
        response_text = self.accept_order.check_response_text()
        ErrorSchema.parse_obj(response_text)
        assert response_status_code == 404 and response_text == {
